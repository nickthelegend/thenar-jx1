...
Pipeline
  ...
  5. write ros2_ws/src/jx1_description/urdf/jx1.urdf (+ jx1.urdf.xacro) and simulation/mujoco/jx1.xml (+ mass report)
...
def write_xacro(links, placeholder):
    """jx1.urdf.xacro: the robot of jx1.urdf with link/joint macros, arguments (mesh prefix, collision set, joint-limit margin,
    joint damping/friction) and actuator-class effort/velocity properties. With default arguments it expands to the same links,
    joints, limits and inertias as jx1.urdf (checked by tools/sim/check_xacro.py)."""
    out = ['<?xml version="1.0"?>',
           '<!-- JX1 humanoid (xacro). Generated by tools/sim/build_robot_description.py from the SolidWorks CAD.',
           '     Default arguments expand to jx1.urdf. Example: xacro jx1.urdf.xacro collision:=visual limit_margin_deg:=2 -->',
           '<robot xmlns:xacro="http://www.ros.org/wiki/xacro" name="jx1">',
           '  <xacro:arg name="mesh_prefix" default="package://jx1_description/meshes"/>',
           '  <xacro:arg name="collision" default="hulls"/>          <!-- hulls: CoACD convex parts | visual: visual mesh | none -->',
           '  <xacro:arg name="limit_margin_deg" default="0"/>       <!-- soft-limit margin: shrinks every joint range at both ends -->',
           '  <xacro:arg name="damping" default="0.05"/>',
           '  <xacro:arg name="friction" default="0.3"/>',
           '  <xacro:property name="mp" value="$(arg mesh_prefix)"/>',
           '  <xacro:property name="col" value="$(arg collision)"/>',
           '  <xacro:property name="margin" value="${float($(arg limit_margin_deg)) * pi / 180.0}"/>',
           '  <!-- actuator classes (actuators/actuator_catalog.yaml): peak torque N m, max speed rad/s -->']
    for c in sorted(EFFORT):
        out.append(f'  <xacro:property name="effort_{c}" value="{EFFORT[c]}"/>')
        out.append(f'  <xacro:property name="velocity_{c}" value="{VELOCITY[c]}"/>')
    out += ['',
            '  <xacro:macro name="jx1_link" params="name mass com ixx ixy ixz iyy iyz izz visual **hulls">',
            '    <link name="${name}">',
            '      <inertial>',
            ...
            '      </inertial>',
            '      <visual><geometry><mesh filename="${mp}/${visual}"/></geometry></visual>',
            '      <xacro:if value="${col == \'visual\'}"><collision><geometry><mesh filename="${mp}/${visual}"/></geometry></collision></xacro:if>',
            '      <xacro:if value="${col == \'hulls\'}"><xacro:insert_block name="hulls"/></xacro:if>',
            '    </link>',
            '  </xacro:macro>',
            '',
            '  <xacro:macro name="jx1_joint" params="name parent child xyz axis lower upper effort velocity">',
            '    <joint name="${name}" type="revolute">',
            '      <parent link="${parent}"/>',
            '      <child link="${child}"/>',
            '      <origin xyz="${xyz}" rpy="0 0 0"/>',
            '      <axis xyz="${axis}"/>',
            '      <limit lower="${lower + margin}" upper="${upper - margin}" effort="${effort}" velocity="${velocity}"/>',
    ...
    for name, L in links.items():
        c, I = L["com"], L["inertia"]
        out.append(f'  <xacro:jx1_link name="{name}" mass="{L["mass"]:.5f}" com="{c[0]:.6f} {c[1]:.6f} {c[2]:.6f}" '
                   f'ixx="{I[0, 0]:.6e}" ixy="{I[0, 1]:.6e}" ixz="{I[0, 2]:.6e}" iyy="{I[1, 1]:.6e}" iyz="{I[1, 2]:.6e}" izz="{I[2, 2]:.6e}" '
                   f'visual="{L["visual"]}">')
        out.append('    <hulls>')
        for h in L["collision"]:
            out.append(f'      <collision><geometry><mesh filename="${{mp}}/{h}"/></geometry></collision>')
        out += ['    </hulls>', '  </xacro:jx1_link>']
    if placeholder:
        tp = TORSO_PLACEHOLDER
        I = np.diag([tp["mass"] / 12 * (tp["box"][1] ** 2 + tp["box"][2] ** 2), tp["mass"] / 12 * (tp["box"][0] ** 2 + tp["box"][2] ** 2),
                     tp["mass"] / 12 * (tp["box"][0] ** 2 + tp["box"][1] ** 2)])
        out += ['  <link name="torso_link">', '    <inertial>', f'      <origin xyz="{tp["com"][0]} {tp["com"][1]} {tp["com"][2]}" rpy="0 0 0"/>',
                f'      <mass value="{tp["mass"]}"/>', f'      <inertia {inertia_xml(I)}/>', '    </inertial>', '  </link>']
    present = set(links) | ({"torso_link"} if placeholder else set())
    out.append('')
    for j in JM["joints"]:
        if j["parent"] not in present or j["child"] not in present:
            continue
        cls = jclass(j["name"])
        o, a = j["origin_xyz_m"], j["axis"]
        out.append(f'  <xacro:jx1_joint name="{j["name"]}" parent="{j["parent"]}" child="{j["child"]}" xyz="{o[0]} {o[1]} {o[2]}" '
                   f'axis="{a[0]} {a[1]} {a[2]}" lower="{j["lower_rad"]}" upper="{j["upper_rad"]}" '
                   f'effort="${{effort_{cls}}}" velocity="${{velocity_{cls}}}"/>')
    out.append('')
    for f in JM["fixed_frames"]:
        if f["parent"] not in present:
            continue
        ...
        out += [f'  <link name="{f["child"]}"/>', f'  <joint name="{f["name"]}" type="fixed">', f'    <parent link="{f["parent"]}"/>',
                f'    <child link="{f["child"]}"/>', f'    <origin xyz="{o[0]} {o[1]} {o[2]}" rpy="0 0 0"/>', '  </joint>']
    out.append('</robot>')
    (PKG_DIR / "urdf" / "jx1.urdf.xacro").write_text("\n".join(out) + "\n", encoding="utf-8")
