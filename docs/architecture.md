...
| Average power | 79 W standing, 204 W walking 0.52 m/s, 289 W at 0.79 m/s | CALCULATED |
| Peak power | 967 W (23 A at 41.6 V) in the fast gait | CALCULATED |
| Runtime | 1.8 h walking, 4.7 h standing (80 % DoD) | CALCULATED |
| Pack | 26 × Samsung 50S (+2 spare), 1.8 kg cells, ₹20/Wh | VERIFIED prices |

## 4. Software architecture (planned)

- `ros2_ws/src/jx1_description` — URDF/Xacro generated from the CAD-derived joint map and mass properties.
- `ros2_control` hardware interface → hub USB protocol (50 Hz targets, 500 Hz state).
- Simulation: MuJoCo MJCF (`simulation/mujoco`), Isaac Sim/Lab via URDF import (`simulation/isaac`).
