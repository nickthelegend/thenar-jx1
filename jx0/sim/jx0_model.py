...
ARM_MASS = {"upper_arm": 0.050, "forearm": 0.040}   # small servo + printed link, each (ASSUMED)
HEAD_MASS = 0.070
...
def build(design: Design | None = None, timestep: float = 0.001) -> str:
  ...
  {_box((0.03, 0.022, 0.03), (0, sgn * 0.012, 0), BLACK)}
  <body name="{side}_upper_arm" pos="0 {sgn * 0.02} 0">
    <inertial pos="0 0 {-la / 2}" mass="{mu}" diaginertia="{mu * la * la / 12:.3g} {mu * la * la / 12:.3g} 1e-5"/>
    ...
    {_box((0.026, 0.026, la), (0, 0, -la / 2))}
    <body name="{side}_forearm" pos="0 0 {-la}">
      <inertial pos="0 0 {-lf / 2}" mass="{mf}" diaginertia="{mf * lf * lf / 12:.3g} {mf * lf * lf / 12:.3g} 1e-5"/>
      <joint name="{side}_elbow" axis="{ax['elbow']}" range="{rng['elbow'][0]:.4f} {rng['elbow'][1]:.4f}" damping="0.01" armature="0.0005"/>
      {_box((0.022, 0.024, 0.024), (0, 0, -0.008), BLACK)}
      {_box((0.02, 0.022, lf), (0, 0, -lf / 2))}
      <geom type="sphere" size="0.014" pos="0 0 {-lf - 0.006}" rgba="{ORANGE}" contype="0" conaffinity="0"/>
    </body>
  </body>
</body>"""
  ...
  <worldbody>
    ...
    <body name="pelvis" pos="0 0 {d.leg_length + 0.002:.5f}">
      ...
      {_box((0.06, 0.13, 0.024), (0, 0, 0.034))}
      ...
      <body name="torso" pos="0 0 0">
        ...
        {_box((0.075, 0.14, 0.13), (0, 0, 0.11))}
        {_box((0.004, 0.10, 0.07), (0.039, 0, 0.12), GREY)}
        {_box((0.03, 0.09, 0.05), (-0.045, 0, 0.10), ".2 .35 .7 1")}
        {arm("l")}
        {arm("r")}
        <body name="head" pos="{_f(neck)}">
          ...
          {_box(hsz, (0, 0, hsz[2] / 2 + 0.005), ORANGE)}
          <geom type="cylinder" size="0.026 0.002" pos="{hsz[0] / 2 + 0.005:.4f} 0 {hsz[2] / 2 + 0.005:.4f}" euler="0 90 0" rgba=".03 .03 .05 1" contype="0" conaffinity="0"/>
          <geom type="sphere" size="0.007" pos="{hsz[0] / 2 + 0.006:.4f} 0.011 {hsz[2] / 2 + 0.012:.4f}" rgba=".3 .85 1 1" contype="0" conaffinity="0"/>
          <geom type="sphere" size="0.007" pos="{hsz[0] / 2 + 0.006:.4f} -0.011 {hsz[2] / 2 + 0.012:.4f}" rgba=".3 .85 1 1" contype="0" conaffinity="0"/>
        </body>
      </body>
      {leg("l")}
